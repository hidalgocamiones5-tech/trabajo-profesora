...
import { Search, Plus, Sparkles, Filter, ChevronRight, ChevronDown, UploadCloud, FileText, Loader2 } from 'lucide-react';
...
export const Compliance = () => {
  ...
  const { normativas, detalleNormativa, isLoading, isLoadingDetalle, error } = useCompliance(selectedNormativa);
    ...
    >
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          {normativas.filter((n: any) => n.progreso > 0).map((normativa: any) => (
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <div className="flex items-center gap-2 font-medium text-slate-700">
                      <div className="w-32 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-lemon-500 rounded-full" style={{ width: `${detalleNormativa?.progreso}%` }}></div>
                      </div>
                      {detalleNormativa?.progreso}%
                    </div>
                  </div>
                )}
              </div>
              ...
              {activeTab === 'Checklist' && (
                <div className="space-y-6">
                  ...
                  {['Diagnóstico Inicial', 'Políticas Internas'].map((categoria) => {
                    const items = (detalleNormativa?.checklist || []).filter((c: any) => c.categoria === categoria);
                    return (
